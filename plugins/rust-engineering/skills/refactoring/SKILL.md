...
Read [Rust Refactoring field guide](references/guide.md) before making a consequential design choice. Use the [Design protocol refactoring helper](./references/guide.md) when a rename, extraction, or move needs symbol/call/trait impact analysis. Keep conclusions tied to the repository's actual toolchain, targets, feature graph, and local instructions.
