> Product adaptation: Retained cloud-native constraint map. Statelessness, environment-only configuration, tracing every request, external storage, Tokio, Axum, Kubernetes, and OpenTelemetry are options, not universal requirements; verify current APIs and deployment semantics before selection.
