> Product adaptation: Retained IoT constraint map. First classify gateway, Linux edge, or MCU; derive buffering, durability, power, security, OTA, and protocol choices from requirements rather than applying them universally. Crate tables are research leads, and the MQTT snippet is a review fragment that must not silently discard publish errors.
...
<!-- rust-example: fragment; missing: selected crates, versions, targets, features, runtime, and project types -->
```rust
