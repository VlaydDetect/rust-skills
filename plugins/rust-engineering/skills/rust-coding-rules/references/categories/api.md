...
|---|---|---|---|
| [`api-builder-must-use`](../rules/api-builder-must-use.md) | `adapted` | `rust-api-design` | Mark builder methods with `#[must_use]` to prevent silent drops |
...
| [`api-default-impl`](../rules/api-default-impl.md) | `adapted` | `rust-api-design` | Implement `Default` for types with sensible default values |
...
| [`api-from-not-into`](../rules/api-from-not-into.md) | `adapted` | `rust-api-design` | Implement `From<T>`, not `Into<U>` - From gives you Into for free |
| [`api-impl-asref`](../rules/api-impl-asref.md) | `adapted` | `rust-api-design` | Use `AsRef<T>` when you only need to borrow the inner data |
| [`api-impl-fromiterator`](../rules/api-impl-fromiterator.md) | `adapted` | `rust-api-design` | Implement `FromIterator` and `Extend` for collection types, and `IntoIterator` for all three reference forms |
| [`api-impl-into`](../rules/api-impl-into.md) | `adapted` | `rust-api-design` | Accept `impl Into<T>` for flexible APIs, implement `From<T>` for conversions |
...
| [`api-operator-overload`](../rules/api-operator-overload.md) | `adapted` | `rust-api-design` | Overload operators only when the semantics are natural and unsurprising |
