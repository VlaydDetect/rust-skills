...
|---|---|---|---|
| [`trait-associated-type-vs-generic`](../rules/trait-associated-type-vs-generic.md) | `adapted` | `rust-traits` | Use an associated type when each impl has exactly one output type; use a generic parameter when a type can implement the trait for many input types |
| [`trait-blanket-impl`](../rules/trait-blanket-impl.md) | `adapted` | `rust-traits` | Use a blanket impl `impl<T: Bound> Trait for T` to give behaviour to every type that satisfies a bound |
| [`trait-coherence-newtype`](../rules/trait-coherence-newtype.md) | `adapted` | `rust-traits` | Respect the orphan rule; wrap a foreign type in a newtype to implement a foreign trait on it |
| [`trait-default-methods`](../rules/trait-default-methods.md) | `adapted` | `rust-traits` | Define a trait in terms of a few required methods plus defaulted ones built on top of them |
...
| [`trait-object-safety`](../rules/trait-object-safety.md) | `adapted` | `rust-traits` | Keep a trait dyn-compatible (object-safe) when you need `dyn Trait` |
