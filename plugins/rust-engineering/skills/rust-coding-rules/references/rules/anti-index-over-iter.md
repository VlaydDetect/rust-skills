...
Apply [`perf-iter-over-index`](perf-iter-over-index.md). This source ID remains addressable for compatibility; the canonical rule owns the decision and verification.
...
<!-- rust-example: fragment; missing: surrounding domain types, functions, imports, and crate context referenced by the When Indices Are Needed illustration -->
```rust
...
<!-- rust-example: fragment; missing: surrounding domain types, functions, imports, and crate context referenced by the Performance Note illustration -->
```rust
