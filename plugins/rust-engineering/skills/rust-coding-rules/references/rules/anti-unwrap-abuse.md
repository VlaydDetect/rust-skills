...
Apply [`err-no-unwrap-prod`](err-no-unwrap-prod.md). This source ID remains addressable for compatibility; the canonical rule owns the decision and verification.
...
<!-- rust-example: fragment; missing: surrounding domain types, functions, imports, and crate context referenced by the When unwrap() Is Acceptable illustration -->
```rust
...
<!-- rust-example: fragment; missing: surrounding domain types, functions, imports, and crate context referenced by the Alternatives to unwrap() illustration -->
```rust
...
<!-- rust-example: fragment; missing: surrounding domain types, functions, imports, and crate context referenced by the expect() Is Slightly Better illustration -->
```rust
// unwrap() - no context
