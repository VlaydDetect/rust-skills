...
- External crates referenced by the source (`tokio-util`, `tokio`, `futures`, `bytes`) must already be accepted by the project or be approved before addition.
...
<!-- rust-example: fragment; missing: surrounding domain types, functions, imports, and crate context referenced by the Patterns for Non-Cancel-Safe Operations illustration -->
```rust
use std::pin::pin;
