...
<!-- rust-example: fragment; missing: surrounding domain types, functions, imports, and crate context referenced by the Hygiene for Local Bindings illustration -->
```rust
macro_rules! swap {
    ($a:expr, $b:expr) => {{
...
fn main() {
    let value = vec![1, 2, 3];
    assert_eq!(sequence_len!(value), 3);
}
```
