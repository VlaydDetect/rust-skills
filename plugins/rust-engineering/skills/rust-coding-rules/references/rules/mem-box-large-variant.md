...
- External crates referenced by the source (`smallvec`, `bytes`) must already be accepted by the project or be approved before addition.
...
<!-- rust-example: fragment; missing: surrounding domain types, functions, imports, and crate context referenced by the Check Enum Sizes illustration -->
```rust
use std::mem::size_of;
...
<!-- rust-example: fragment; missing: surrounding domain types, functions, imports, and crate context referenced by the Recursive Types Require Boxing illustration -->
```rust
...
<!-- rust-example: fragment; missing: surrounding domain types, functions, imports, and crate context referenced by the Pattern Matching with Boxed Variants illustration -->
```rust
enum Event {
    Small(u32),
    Large(Box<LargeData>),
