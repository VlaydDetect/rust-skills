...
Consider this rule only after its prerequisites are satisfied: Use `with_capacity()` when size is known.
...
- External crates referenced by the source (`smallvec`) must already be accepted by the project or be approved before addition.
...
<!-- rust-example: fragment; missing: surrounding domain types, functions, imports, and crate context referenced by the Collection Capacity Methods illustration -->
```rust
// Vec
...
<!-- rust-example: fragment; missing: surrounding domain types, functions, imports, and crate context referenced by the Estimating Capacity illustration -->
```rust
...
From fd (file finder):
<!-- rust-example: fragment; missing: surrounding domain types, functions, imports, and crate context referenced by the Evidence from Production Code illustration -->
```rust
...
<!-- rust-example: fragment; missing: surrounding domain types, functions, imports, and crate context referenced by the When to Skip illustration -->
```rust
