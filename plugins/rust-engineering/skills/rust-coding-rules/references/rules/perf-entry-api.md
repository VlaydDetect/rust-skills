...
<!-- rust-example: fragment; missing: surrounding domain types, functions, imports, and crate context referenced by the Pattern: Count Occurrences illustration -->
```rust
...
<!-- rust-example: fragment; missing: surrounding domain types, functions, imports, and crate context referenced by the Pattern: Group By illustration -->
```rust
...
<!-- rust-example: fragment; missing: surrounding domain types, functions, imports, and crate context referenced by the Pattern: Complex Entry Logic illustration -->
```rust
match map.entry(key) {
...
fn main() {
    let mut counts = HashMap::new();
    *counts.entry("rust").or_insert(0_u32) += 1;
