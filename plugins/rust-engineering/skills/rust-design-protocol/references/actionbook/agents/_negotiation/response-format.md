# DesignBrief response format

> Adapted from Actionbook's negotiation response. Use it for material comparisons or cross-layer uncertainty, not as mandatory boilerplate.

## When to Use

Use the smallest subset that preserves constraints, evidence, alternatives, confidence, and gaps. `rust-design-protocol/references/design-brief.md` is the canonical product contract.
