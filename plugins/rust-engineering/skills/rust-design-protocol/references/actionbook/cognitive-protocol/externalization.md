...
> Product adaptation: Use task artifacts only for long, inspectable work and only within authorized scope. Record decisions, evidence, attempts, and status, not private reasoning traces; no fixed two-action write cadence.
...
Filesystem = durable task state (persistent but scoped and finite)
...
> For a long task, refresh an existing evidence artifact when a material decision or state change would otherwise be lost.
