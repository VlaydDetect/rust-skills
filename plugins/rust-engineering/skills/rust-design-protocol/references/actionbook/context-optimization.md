...
> Product adaptation: retain progressive disclosure and isolation heuristics, but do not force context forks, three-agent parallelism, fixed token estimates, or host-specific commands. Load one needed branch and delegate only bounded independent read-only work.
