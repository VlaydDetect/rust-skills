...
> Adapted from Actionbook `meta-cognition-parallel`. The three layers are analysis lenses, not a requirement to launch three agents. Parallel read-only work is reserved for independent, high-value questions.
...
*Report the decision trace and evidence, not private chain-of-thought. Parallel analysis is optional.*
```
