...
Read [Rust Error Design field guide](references/guide.md) before making a consequential design choice. Use the [Design protocol error protocol](./references/guide.md) when failure must be traced across mechanical, API, and domain layers. Keep conclusions tied to the repository's actual toolchain, targets, feature graph, and local instructions.
