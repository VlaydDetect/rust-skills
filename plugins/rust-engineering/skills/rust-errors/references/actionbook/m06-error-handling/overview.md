...
> Adapted from Actionbook `m06-error-handling`. Error policy follows the caller contract; neither `expect`, `unwrap`, `anyhow`, nor `thiserror` is a universal preference.
...
|--------------|---------|--------|
| `.unwrap()` everywhere | Panics on recoverable input | Propagate with `?`, handle explicitly, or use `expect` only for a documented invariant |
