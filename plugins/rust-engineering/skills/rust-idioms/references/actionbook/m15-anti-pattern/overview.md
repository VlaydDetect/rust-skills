...
> Adapted from Actionbook `m15-anti-pattern`. Diagnose the violated contract and smallest correction; a pattern name alone is not a finding.
...
| `.unwrap()` on recoverable production input | Runtime panic | Propagate with `?`, handle explicitly, or use `expect` only for a documented invariant |
