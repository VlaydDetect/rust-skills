...
> Adapted from Actionbook `rust-code-navigator`. Use an existing Graphify index first when present, then host-native LSP when available, and confirm results in current source.
...
|-------|-------|----------|
| "No LSP server" | rust-analyzer unavailable | Fall back to Graphify and `rg`; installation requires an explicit user request |
