...
> Adapted from Actionbook `rust-deps-visualizer`. Resolve the effective graph through locked offline Cargo metadata; do not infer dependencies by grepping manifests.
...
```bash
cargo tree --locked --offline --depth 3
```
