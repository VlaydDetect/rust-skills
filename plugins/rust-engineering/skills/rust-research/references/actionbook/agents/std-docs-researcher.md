...
> Adapted into `rust-researcher`. Match standard-library documentation to the project's toolchain/MSRV and keep evidence in the current result.
...
Do not write a home-directory cache. Persist only when the user explicitly requests a project-local dossier or documentation update.
