...
> Product adaptation: retained for cache-key, provenance, and invalidation considerations only. Do not create the source global cache, use its fixed TTLs, or expose its commands. The product default is read-only research; explicitly requested crate sync writes a project-local exact-version Cargo-metadata dossier.
