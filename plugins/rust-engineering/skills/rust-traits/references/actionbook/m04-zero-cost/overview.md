...
> Adapted from Actionbook `m04-zero-cost`. Treat "zero cost" as an implementation model to verify, not a promise that every generic abstraction is free in code size or compile time.
