...
> Review corpus only: these retained source fragments predate the product's
> Rust 2024 baseline and are not paste-ready. Update foreign blocks and exported
> attributes for the selected edition, omit the invalid negative `Clone` impl,
> and independently prove callback lifetime, pinning, concurrent invocation,
> reentrancy, panic behavior, handle auto traits, allocator pairing, and
> target-specific layout. The final hard-coded `Config` layout is not portable
> across all targets.
