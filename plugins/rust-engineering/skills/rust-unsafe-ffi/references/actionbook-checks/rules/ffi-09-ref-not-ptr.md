...
> Product adaptation for `ffi-09`: A reference-based wrapper is safe only when the callee cannot retain, mutate beyond the borrow, race, or violate validity. Raw-pointer parameters can remain in a safe API when the function performs no unsafe operation on them.
>
