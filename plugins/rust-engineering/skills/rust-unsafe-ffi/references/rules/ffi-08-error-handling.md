...
> Rule refinement for `ffi-08`: Use an explicitly documented ABI representation. Validate output pointers and lengths before writing; an exported Rust function with caller safety obligations should be unsafe from Rust callers even though C cannot express that marker.
>
> The retained Design protocol examples are review fragments, not universal policy or compile-tested golden code. Apply the repository MSRV, edition, targets, headers, and foreign runtime contract.
...
<!-- rust-example: fragment; missing: headers, defining types, target ABI, ownership, and foreign runtime context -->
```rust
