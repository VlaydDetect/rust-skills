...
> Product adaptation: select applicable rows. A missing optional tool run is
> not automatically a defect; an unproven safety precondition is.
...
- [ ] **Non-null**: When the operation requires it, is non-nullness proved before dereference or reference construction?
...
- [ ] If Miri supports this code path and target assumptions, has a focused run been considered or recorded?
...
| Severity | Requires |
|----------|----------|
| `transmute` | Explicit representation proof; extra review if repository policy requires it |
| Manual `Send`/`Sync` | Complete thread and destruction proof; extra review if repository policy requires it |
