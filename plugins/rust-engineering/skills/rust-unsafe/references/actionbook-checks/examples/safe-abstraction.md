...
> Review corpus only: these retained source examples are not compile-tested
> golden code. In particular, the negative `Clone` impl is not a valid stable
> impl, Edition 2024 requires explicit unsafe blocks inside `unsafe fn`, the
> pointer-range iterator needs a zero-sized-type design, and the callback-free
> `Send` proof and array builder omit production invariants. Prefer safe std
> constructors such as `std::array::from_fn` when they meet the task.
