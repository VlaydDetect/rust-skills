...
> Product adaptation for `io-01`: A stale handle is not categorically Rust UB, but violating an unsafe I/O-safety contract can access a reused resource. Prefer OwnedFd or BorrowedFd and their Windows analogues when ownership permits.
>
> The retained Actionbook examples are review fragments, not universal policy or compile-tested golden code. Apply the repository MSRV, edition, targets, and local invariants.
