...
> Product adaptation: error codes are entry signals, not sufficient routing evidence. Map m01 and m03 to `rust-ownership`, m04 to `rust-traits`, m07 to `rust-concurrency`, and diagnose m11-style resolution failures with `rust-navigation`, `rust-cargo-build`, or `rust-ecosystem` according to the actual cause. Every "Common Fix" is a hypothesis; never default to clone, RefCell, Arc, Mutex, dyn, or a new dependency.
...
<!-- rust-example: fragment; missing: imports, defining types, compiler output, and repository context -->
```rust
